...
export function getNewShapePosition(source, element) {
  ...
  if (isAny(element, ["bpmn:Token", "bpmn:ProcessSnapshot"])) {
    return getTokenPosition(source, element);
  }
...
/**
 * Get the position for given token. Try placing the token
 ...
 * @return {Point}
 */
export function getTokenPosition(source, element) {
  const sourceTrbl = asTRBL(source);

  let position = {
    x: sourceTrbl.left + source.width / 2,
    y: sourceTrbl.bottom + 3 * element.height,
  };
