...
ContextPadProvider.prototype.getContextPadEntries = function (element) {
  ...
  const attachToken = function (event, element) {
    const shape = elementFactory.createShape(
      assign({ type: "token:Token" }, translate("Add token")),
    );
    console.log("Attach token finished", shape);
  };
  if (isAny(businessObject, ["bpmn:Activity", "bpmn:SequenceFlow"])) {
    assign(actions, {
      "append.text-annotation2": appendAction(
          "bpmn:TextAnnotation2",
          "bpmn-icon-bpmn-io",
          translate("Add token"),
      ),
    });
