...
import { forEach, filter, isArray, isUndefined } from "min-dash";
...
ReplaceMenuProvider.prototype.getPopupMenuEntries = function (target) {
  ...
  if (
    ...
    !is(target.parent, "bpmn:Collaboration")
  ) {
    ...
    filteredReplaceOptions = filter(
      ...
      function (replaceOption) {
        ...
        var isInterruptingEqual =
          businessObject.isInterrupting === isInterrupting;

        // filters elements which types and event definition are equal but have have different interrupting types
        return (
          differentType(replaceOption) ||
          (!differentType(replaceOption) && !isInterruptingEqual)
        );
      },
    );
...
ReplaceMenuProvider.prototype._createEntries = function (
  ...
  forEach(replaceOptions, function (replaceOption) {
    entries[replaceOption.actionName] = self._createEntry(
      replaceOption,
      target,
    );
  });
    ...
    switch (replaceOption.actionName) {
      case "replace-with-default-flow":
        if (
          ...
          entries = {
            ...entries,
            [replaceOption.actionName]: self._createEntry(
              replaceOption,
              target,
              function () {
 ...
 */
ReplaceMenuProvider.prototype._createEntry = function (
  replaceOption,
  target,
  action,
) {
 ...
 * @return {PopupMenuHeaderEntries}
 */
ReplaceMenuProvider.prototype._getLoopCharacteristicsHeaderEntries = function (
  target,
) {
  var self = this;
  var translate = this._translate;
  ...
  function toggleLoopEntry(event, entry) {
    ...
    if (entry.active) {
      ...
      if (
        isUndefined(entry.options.isSequential) ||
 ...
 * @return {PopupMenuHeaderEntries}
 */
ReplaceMenuProvider.prototype._getParticipantMultiplicityHeaderEntries =
  function (target) {
    var self = this;
