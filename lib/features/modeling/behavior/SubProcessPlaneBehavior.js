...
export default function SubProcessPlaneBehavior(
  ...
  function removeRoot(context) {
    ...
    var rootElement = (context.newRootElement = elementRegistry.get(
      getPlaneIdFromShape(businessObject),
    ));
  ...
  this.preExecuted(
    ...
    function (context) {
      ...
      if (
        !isCollapsedSubProcess(oldShape) ||
        !isCollapsedSubProcess(newShape)
      ) {
        return;
      }
 ...
 * @param {Root} target
 */
SubProcessPlaneBehavior.prototype._moveChildrenToShape = function (
  source,
  target,
) {
  ...
  children = children.concat(
    children.reduce(function (labels, child) {
  ...
  if (!visibleChildren.length) {
    modeling.moveElements(children, { x: 0, y: 0 }, target, {
      autoResize: false,
    });
    return;
  }
 ...
 * @return {Element[]}
 */
SubProcessPlaneBehavior.prototype._showRecursively = function (
  elements,
  hidden,
) {
  var self = this;
