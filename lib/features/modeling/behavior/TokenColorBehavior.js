...
export default function TokenColorBehavior(eventBus, modeling) {
  ...
  function removeConnectionChangeColorUpdateModdle(token, snapshot, context) {
  ...
  this.postExecuted(
    ...
    function (context) {
      ...
      if (is(source, "bt:Token") && is(target, "bt:ProcessSnapshot")) {
        removeConnectionChangeColorUpdateModdle(source, target, context);
      }
      if (is(target, "bt:Token") && is(source, "bt:ProcessSnapshot")) {
        removeConnectionChangeColorUpdateModdle(target, source, context);
      }
    },
    true,
  );
