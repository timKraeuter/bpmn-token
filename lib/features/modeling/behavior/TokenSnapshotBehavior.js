 ...
 */
export default function TokenSnapshotBehavior(eventBus, modeling) {
  ...
  function findAllTokens(root) {
    return root.children
      .filter((child) => is(child, "bpmn:Participant"))
      .flatMap((child) => child.children)
      .filter((child) => is(child, "bt:Token"));
  }
  ...
  function isLinkedToSnapshot(token, deletedSnapshot) {
    return (
      token.businessObject.processSnapshot &&
      token.businessObject.processSnapshot.id === deletedSnapshot.id
    );
  }
